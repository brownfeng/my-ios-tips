...
struct Chat {
    ...
    static func mockData() -> [Self] {
        ...
        let data2 = (0...2).enumerated().map { _, _ in
            return Chat(title: "brownfeng", date: "2022.4.23", memberSince: "还闭包作为一个实际参数传递给一个函数的时候，并且它会在函数返回之后调用我们就说这个闭包逃逸了，当你声明一个接受闭包作为形式参数的函数时，你可以在形式参数前写@escaping来明确闭包是允许闭包作为一个实际参数传递给一个函数的时候，并且它会在函数返回之后调用我们就说这个闭包逃逸了，当你声明一个接受闭包作为形式参数的函数时，你可以在形式参数前写@escaping来明确闭包是允许.", description: """
                                当闭包作为一个实际参数传递给一个函数的时候，并且它会在函数返回之后调用我们就说这个闭包逃逸了，当你声明一个接受闭包作为形式参数的函数时，你可以在形式参数前写@escaping来明确闭包是允许逃逸的。
        ...
        let data3 = (0...2).enumerated().map { _, _ in
            return Chat(title: "brownfeng", date: "2022.4.23", memberSince: "还闭包作为确闭包是允许.", description: """
