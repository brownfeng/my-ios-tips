...
class ImageLoadOperation: AsyncOperation {
  ...
  var outputImage: UIImage? // 构造成 output 服务
...
class TiltShiftOperation: Operation {
  ...
  override func main() {
      // 通过 dependencies 过滤出 FilterDataProvider!!! 这个就是
      // 然后获取 依赖的 outputImage
...
// 防止直接定义一个包含 outputImage 的服务, 这里创建一个 protocol, 对象可能有一个 outputImage 属性
protocol FilterDataProvider {
  var outputImage: UIImage? { get }
}

//  Filter 数据提供者!!! 实际拥有一个 output
